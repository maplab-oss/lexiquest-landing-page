...
export const InfoSectionOne = ({ id }: Props) => {
  return (
    <section id={id} className="bg-white py-12">
      <div className="section-container">
        <div className="grid grid-cols-1 items-start gap-1 lg:grid-cols-2">
          {/* Feature Mockup */}
          <SpringInView className="relative max-w-[375px] lg:max-w-[500px]">
            <div className="shadow-3xl relative flex flex-col items-center overflow-hidden rounded-3xl rounded-br-none bg-green-100 p-8 pb-0">
              <img
                src={contentConfig.assets.mockups.heroSectionOne}
                loading="lazy"
                alt="Language learning mockup"
                className="max-h-96 w-full rounded-t-3xl object-cover object-top"
              />
            </div>
            <div className="absolute -top-[5%] -right-[5%] max-w-[60%] rounded-3xl rounded-br-none bg-green-400 p-5 lg:p-8">
              <div className="flex w-full flex-col space-y-3">
                {contentConfig.infoSectionOne.checklist.map((item, index) => (
          ...
          <div>
            <SpringInView>
              <h2 className="heading-l2">
                {contentConfig.infoSectionOne.title}
              </h2>
            </SpringInView>

            {contentConfig.infoSectionOne.features.map((feature, index) => (
